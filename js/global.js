...
var site = new Object();
site.Name = '浙江省方正校准有限公司';
site.Name2 = 'FZ';
site.Url = '';
site.SiteID = '';
site.Dir = '/';
site.CookieDomain = '';
site.CookiePrev = 'gscmsV110';
site.AllowReg = true;
site.CheckReg = false;
...
{recordcount: 7, table: [{no: 0, id: 1, title: '文章', type: 'article', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 1, id: 2, title: '图片', type: 'photo', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 2, id: 3, title: '下载', type: 'soft', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 3, id: 4, title: '视频', type: 'video', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 4, id: 5, title: '文档', type: 'document', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 5, id: 6, title: '文库', type: 'paper', fieldvalues: 'title,summary', fieldtexts: '标题,简介'},{no: 6, id: 7, title: '产品', type: 'product', fieldvalues: 'title,summary', fieldtexts: '标题,简介'}]}
...
{recordcount: 3, table: [{title: 'QQ账号', code: 'qq',enabled: 0},{title: '新浪微博', code: 'sina',enabled: 0},{title: '人人网账号', code: 'renren',enabled: 0}]}
...
var _gscms_Host = function(){
    if(document.URL.substr(0,7)=='http://')
        return 'http://'+window.location.host;
    else
        return 'https://'+window.location.host;
}
var _gscms_DialogUrl = site.Dir + "statics/dialog/";
...
var GSCMS=new Object();
var hexcase = 0;//1为大写
var chrsz   = 8;  
    
GSCMS.MD5=function(s){
	if(s.length == 32)
		return s;
	return _gscms_binl2hex(_gscms_core_md5(_gscms_str2binl(s), s.length * chrsz));
}   

function _gscms_core_md5(x, len)   
{   
  ...
  {   
    ...
    a = _gscms_safe_add(a, olda);   
    b = _gscms_safe_add(b, oldb);   
    c = _gscms_safe_add(c, oldc);   
    d = _gscms_safe_add(d, oldd);   
  }   
...
function _gscms_md5_cmn(q, a, b, x, s, t)   
{   
  return _gscms_safe_add(_gscms_bit_rol(_gscms_safe_add(_gscms_safe_add(a, q), _gscms_safe_add(x, t)), s),b);   
}   
function md5_ff(a, b, c, d, x, s, t)   
{   
  return _gscms_md5_cmn((b & c) | ((~b) & d), a, b, x, s, t);   
}   
function md5_gg(a, b, c, d, x, s, t)   
{   
  return _gscms_md5_cmn((b & d) | (c & (~d)), a, b, x, s, t);   
}   
function md5_hh(a, b, c, d, x, s, t)   
{   
  return _gscms_md5_cmn(b ^ c ^ d, a, b, x, s, t);   
}   
function md5_ii(a, b, c, d, x, s, t)   
{   
  return _gscms_md5_cmn(c ^ (b | (~d)), a, b, x, s, t);   
}   
 
function _gscms_core_hmac_md5(key, data)   
{   
  var bkey = _gscms_str2binl(key);   
  if(bkey.length > 16) bkey = _gscms_core_md5(bkey, key.length * chrsz);   
  ...
  var hash = _gscms_core_md5(ipad.concat(_gscms_str2binl(data)), 512 + data.length * chrsz);   
  return _gscms_core_md5(opad.concat(hash), 512 + 128);   
}   
  
  
function _gscms_safe_add(x, y)   
{   
...
function _gscms_bit_rol(num, cnt)   
{   
  return (num << cnt) | (num >>> (32 - cnt));   
}   
  
  
function _gscms_str2binl(str)   
{   
  var bin = Array();   
  var mask = (1 << chrsz) - 1;   
...
function _gscms_binl2hex(binarray)   
{   
...
GSCMS.Cookie={//
    set:function(name,value,expires,path,domain){
        ...
        document.cookie=name+"="+_gscms_UrlEncode(value)+((expires)?"; expires="+expires.toGMTString():"")+((path)?"; path="+path:"; path=/")+((domain!=null && domain.length>0)?";domain="+domain:"");
    },
    get:function(name, subname){
        var re = new RegExp((subname ? name + "=(.*?&)*?" + subname + "=(.*?)(&|;)" : name + "=([^;]*)(;|$)"), "i");
        return _gscms_UrlDecode(re.test(document.cookie) ? (subname ? RegExp["$2"] : RegExp["$1"]) : ""); 
    }, 
...
//追加onload事件
GSCMS.addOnloadEvent=function(fnc) {
...
GSCMS.isFunction=function(variable) {return (typeof(variable) == 'function' ? true : false);};
GSCMS.isUndefined=function(variable) {return (typeof(variable) == 'undefined' ? true : false);};
/*字节长度*/
GSCMS.Length=function(variable) {
    var len = 0;
    var val = variable;
...
//弹出消息框
/////////////////////////////
GSCMS.Message=function(errstr, success, returnFunc){
    var MSG = $.message;
    MSG.lays(200, 24,_gscms_GetScrollTop()+2);
    //MSG.anim('fade', 'slow');
    MSG.anim('fade', 'slow', site.Dir + '_libs/jquery.messager/');
    if(returnFunc) MSG.doafter(returnFunc);
    MSG.show(success, errstr, 1500);
    new _gscms_Dialog().reset();//消息显示完毕完再执行
};
/////////////////////////////
//弹出提示框
/////////////////////////////
GSCMS.Alert=function(errstr, success, returnFunc){
    var oDialog = new _gscms_Dialog('2', '', 360, 180, success, true);
    oDialog.init();
...
//弹出确认框
//例如:
//1、GSCMS.Confirm("是否操作", act, null) //函数不加()
//2、GSCMS.Confirm("是否操作", "alert('yes')", "alert('no')")
/////////////////////////////
GSCMS.Confirm=function(errstr, returnSubmitFunc, returnCancelFunc)
{
    var oDialog = new _gscms_Dialog('2', '', 360, 180, null, true);
    oDialog.init();
    oDialog.event(errstr,'');
...
GSCMS.Popup={
    ...
    {
        new _gscms_Dialog().reset();
        if(showTitle==null) showTitle="&nbsp;";
        var oDialog = new _gscms_Dialog('2', showTitle, width, height, null, showCloseBox);
        if (url.indexOf("?") == -1)
    ...
    hide:function(callReturnFunc){
        new _gscms_Dialog().reset(callReturnFunc);
    }
};
/////////////////////////////
//弹出加载层
...
GSCMS.Loading={
    show:function(msgstr, width, height)
    {
        ...
        var oDialog = new _gscms_Dialog('0', '友情提示', width, height, null, false);
        oDialog.init(true);
        oDialog.html("<div style='text-align:center;padding-top:20px;'>"+msgstr+"<br /><br /><img src='" + _gscms_DialogUrl + "loading.gif' align='absmiddle'></div>");
    },
    hide:function(callReturnFunc){
        new _gscms_Dialog().reset(callReturnFunc);
    }
};
GSCMS.Event.add(window,"load",_gscms_OperatorPlus);
GSCMS.Event.add(window,"scroll",_gscms_OperatorPlus);
GSCMS.Event.add(window,"resize",_gscms_OperatorPlus);
...
GSCMS.Core = {
    WriteUserInfo: function () {
        $.ajax({
            type: "get",
            dataType: "json",
            url: _gscms_Host() + site.Dir + "ajax/user.aspx?oper=ajaxLoginbar&clienttime=" + Math.random(),
    ...
    ShowWeather: function (obj) {
        $.ajax({
            ...
            url: _gscms_Host() + site.Dir + "plus/weather/json.aspx",
    ...
    AddDigg: function (cType, id) {
        $.ajax({
            ...
            url: _gscms_Host() + site.Dir + "digg/ajax.aspx",
            error: function (XmlHttpRequest, textStatus, errorThrown) { if (XmlHttpRequest.responseText != "") { alert(XmlHttpRequest.responseText); } },
            success: function (d) {
...
//标题栏跑马灯
////////////////////////////////////////////////////////////////////////////////////////////
var _gscms_ScrollTitle__Oldtitle        = top.document.title;
var _gscms_ScrollTitle__i        = 0;
var _gscms_ScrollTitle__Speed        = 200;
var _gscms_ScrollTitle__Timer        = function(message){
    if(_gscms_ScrollTitle__i == message.length)
    {
        top.document.title = _gscms_ScrollTitle__Oldtitle;
        _gscms_ScrollTitle__i = 0;
        return;
    }
    else{
        top.document.title = message.substring(_gscms_ScrollTitle__i);
        _gscms_ScrollTitle__i++;
        setTimeout("_gscms_ScrollTitle__Timer('"+message+"')",_gscms_ScrollTitle__Speed);
    }
}

////////////////////////////////////////////////////////////////////////////////////////////

var _gscms_HideSelects            = false;
var _gscms_DialogIsShown            = false;
var _gscms_WindowMask            = null;
////////////////////////////////////////////////////////////////////////////////////////////
//以下为弹出窗口的类
////////////////////////////////////////////////////////////////////////////////////////////
function _gscms_Dialog(styletype, title, width, height, iswhat, showCloseBox){
    //半透明边框宽度
    var shadowBorderBoth=0;
    var oWidth = width;
    var oHeight = height;
    if(oWidth==-1 || oWidth>_gscms_GetViewportWidth()-15)
    {
        oWidth=_gscms_GetViewportWidth()-15;
        shadowBorderBoth = 0;
    }
    if(oWidth<-1)
    {
        oWidth=_gscms_GetViewportWidth()+oWidth;
        shadowBorderBoth = 0;
    }
    if(oHeight==-1 || oHeight>_gscms_GetViewportHeight()-44)
    {
        oHeight=_gscms_GetViewportHeight()-44;
        shadowBorderBoth = 0;
    }
    if(oHeight<-1)
    {
        oHeight=_gscms_GetViewportHeight()+oHeight;
        shadowBorderBoth = 0;
    }
    var sTitle = "友情提示";
    if (iswhat == "0")
    ...
    var src = "";
    var path = _gscms_DialogUrl + styletype + "/";
    var gReturnFunc;
    var gReturnVal = null;
    var sButtonFunc = '<input id="dialogSubmit" class="dialogSubmit' + styletype + '" type="button" value="确 认" onclick="new _gscms_Dialog().reset();" /> <input id="dialogCancel" class="dialogCancel' + styletype + '" type="button" value="取 消" onclick="new _gscms_Dialog().reset();" />';
    var sClose = '';
    if (showCloseBox == null || showCloseBox == true)
        sClose = '<img alt="关闭" style="cursor:pointer;" id="dialogBoxClose" onclick="new _gscms_Dialog().reset();" src="' + path + 'dialogCloseOut.gif" border="0" onmouseover="this.src=\'' + path + 'dialogCloseOver.gif\';" onmouseout="this.src=\'' + path + 'dialogCloseOut.gif\';" align="absmiddle" />';
    var sSuccess = '';
    if (iswhat != null)
	...
	document.body.style.overflow='hidden';//屏蔽滚动条
        ...
        _gscms_WindowMask = $i("windowMask");
        _gscms_WindowMask.style.display="block";
        var brsVersion = parseInt(window.navigator.appVersion.charAt(0), 10);
        if (brsVersion <= 6 && window.navigator.userAgent.indexOf("MSIE") > -1) {
            _gscms_HideSelects = true;
        }
        if (_gscms_HideSelects == true) {
            HideSelectBoxes();
        }
    ...
    //this.show = function(){$i('dialogBox') ? function(){} : this.init();_gscms_DialogIsShown=true;this.middle('dialogBox');}
    this.show = function(){$i('dialogBox') ? function(){} : this.init();_gscms_DialogIsShown=true;this.middle('dialogBox');this.shadow();this.middle('dialogBoxShadow');_gscms_OperatorPlus();}
...
///////////////////////////////////////////////////////////////////////////
function _gscms_OperatorPlus() {
    if (_gscms_DialogIsShown == true) {
        var oDialogDiv = $i("dialogBox");
        var oShadow = $i("dialogBoxShadow");
        var oWidth = oDialogDiv.offsetWidth;
        var oHeight = oDialogDiv.offsetHeight;
        var theBody = document.getElementsByTagName("BODY")[0];
        var scTop = parseInt(_gscms_GetScrollTop(),10);
        var scLeft = parseInt(theBody.scrollLeft,10);
        var fullHeight = _gscms_GetViewportHeight();
        var fullWidth = _gscms_GetViewportWidth();
        ...
        if (_gscms_WindowMask != null) {
            ...
            _gscms_WindowMask.style.height = popHeight + "px";
            _gscms_WindowMask.style.width = popWidth + "px";
        }
    }
}
function _gscms_GetViewportHeight() {
...
function _gscms_GetViewportWidth() {
    var offset = 17;
    var width = null;
...
function _gscms_GetScrollTop() {
    if (self.pageYOffset){return self.pageYOffset;}
    else if (document.documentElement && document.documentElement.scrollTop){return document.documentElement.scrollTop;}
    else if (document.body){return document.body.scrollTop;}
}
function _gscms_GetScrollLeft() {
    if (self.pageXOffset){return self.pageXOffset;}
    else if (document.documentElement && document.documentElement.scrollLeft){return document.documentElement.scrollLeft;}
    else if (document.body){return document.body.scrollLeft;}
}
function _gscms_SetDialogTitle(){
    ...
    $(document).keyup(function(e) {
        ...
        if (key == 27) {
            parent.GSCMS.Popup.hide(); 
        }
    }); 
}
function _gscms_SetDialogSize(w,h){
    try {
...
/* Url编码 */ 
function _gscms_UrlEncode(unzipStr){ 
    var zipstr=""; 
    var strSpecial="!\"#$%&'()*+,/:;<=>?[]^`{|}~%"; 
    var tt= ""; 
    for(var i=0;i<unzipStr.length;i++){ 
        var chr = unzipStr.charAt(i); 
        var c=_gscms_StringToAscii(chr); 
        tt += chr+":"+c+"n"; 
...
/* Url解码 */ 
function _gscms_UrlDecode(zipStr){ 
    var uzipStr=""; 
    for(var i=0;i<zipStr.length;i++){ 
        ...
        if(chr == "+"){ 
            ...
            if(parseInt("0x"+asc)>0x7f){ 
                ...
                uzipStr+=_gscms_AsciiToString(parseInt("0x"+asc)); 
                i+=2; 
            } 
        }else{ 
            uzipStr+= chr; 
...
var _gscms_StringToAscii = function(str){return str.charCodeAt(0).toString(16);}
var _gscms_AsciiToString = function(asccode){return String.fromCharCode(asccode);}

////////////////////////////////////////////////////////////////////////////////////////////
function _gscms_SetUrlRefresh(url)
{
    if(url.indexOf("?") > 0)
...
/*刷新验证码*/
function _gscms_GetRefreshCode(obj, h){
    if(!h)
        $i(obj).src=_gscms_SetUrlRefresh(site.Dir+"plus/getcode.aspx");
    else
        $i(obj).src=_gscms_SetUrlRefresh(site.Dir+"plus/getcode.aspx?h="+h);
}
/*当前网站访问者信息*/
var user = new Object();
if(GSCMS.Cookie.get(site.CookiePrev + "user", "id") == ""){//游客
    user.id = "0";
    user.name = "guest";
    user.userkey = "666666";
    user.groupid = "0";
    ...
    user.id = GSCMS.Cookie.get(site.CookiePrev + "user", "id");
    user.name = GSCMS.Cookie.get(site.CookiePrev + "user", "name");
    user.userkey = GSCMS.Cookie.get(site.CookiePrev + "user", "userkey");
    user.groupid = GSCMS.Cookie.get(site.CookiePrev + "user", "groupid");
    user.adminid = GSCMS.Cookie.get(site.CookiePrev + "user", "adminid");
    user.groupname = GSCMS.Cookie.get(site.CookiePrev + "user", "groupname");
    user.cookies = GSCMS.Cookie.get(site.CookiePrev + "user", "cookies");
}

var _gscms_StuHover = function() {
    var cssRule;
    var newSelector;
...
function InsertUnit(text, obj) {
    ...
    var o = $i(obj);
    o.focus();
    if(!GSCMS.isUndefined(o.selectionStart)) {
...
function ajaxPlayCodeVoice(wmpid)
{
    ...
    $.ajax({
        ...
        dataType:    "json",
        data:        "clienttime="+Math.random(),
        url:        _gscms_Host() + site.Dir + "ajax/other.aspx?oper=ajaxGetValidateCode",
        error:        function(XmlHttpRequest,textStatus, errorThrown){if(XmlHttpRequest.responseText!=""){alert(XmlHttpRequest.responseText);}},
		success:	function(d){
...
/*{begin第三方接口*/
function _gscms_GetOAuthBar(){
    var _oauthbar="";
    var _oauths = ___JSON_OAuths.table;
    for(var i=0;i<_oauths.length;i++){
        if(_oauths[i].enabled==1)
            _oauthbar += '<li><a href="'+site.Dir+'app/'+_oauths[i].code+'/oauth2.aspx?type=login&mode=thirdlogin"><img src="'+site.Dir+'statics/passport/'+_oauths[i].code+'16.gif" title="使用'+_oauths[i].title+'登录"  alt="使用'+_oauths[i].title+'登录" /></a></li>';
    }
    return _oauthbar;
}
//发送微博
function _gscms_SendWeiboUrl(msg, url, oauth){
	if(url.substr(0,7)!='http://' && url.substr(0,8)!='https://')
		url=site.Url+url;
	return site.Dir+'app/'+oauth+'/weibo.aspx?msg='+encodeURIComponent(msg+' '+url);
}
//绑定链接
function _gscms_SetOAuthBar(){
    var _oauths = ___JSON_OAuths.table;
    var _enablednum=0;
    for(var i=0;i<_oauths.length;i++){
        if(_oauths[i].enabled==1){
            _enablednum++;
            $('#oauth_'+_oauths[i].code).attr("title","使用"+_oauths[i].title+"登录").attr("href",site.Dir+"app/"+_oauths[i].code+"/oauth2.aspx?type=login&mode=thirdlogin");
        }
        else
...
// 显示当前日期
function setCurrentDateTime2(o) {
    var d = new Date();
    var da = d.getDate();
    var mo = d.getMonth() + 1;
    var y = d.getFullYear();
    var week = ['天', '一', '二', '三', '四', '五', '六'];
    if (typeof (o) != 'object') { o = $i(o) }
    o.innerHTML = '今天是：' + y + '年' + mo + '月' + da + '日 星期' + week[d.getDay()];
    window.setTimeout(function () { setCurrentDateTime2(o) }, 1000);
}
...
function addFavorite(ccid,cType,id)
{
    $.ajax({
        ...
        url:        _gscms_Host() + site.Dir + "ajax/content.aspx?oper=ajaxAddFavorite",
        error:        function(XmlHttpRequest,textStatus, errorThrown){if(XmlHttpRequest.responseText!=""){alert(XmlHttpRequest.responseText);}},
		success:	function(d){
			switch (d.result)
			{
...
function jTab(Id, tId, EclassName, iBeHavior){
    ...
    this.showDiv=function(tDiv){
        if(document.getElementById(tId) && document.getElementById(tDiv)){
            ...
            document.getElementById(tId + "a").innerHTML = document.getElementById(tDiv + "a").outerHTML;
...
/*加入收藏夹*/
function _gscms_addFavorite(A,C)
{
...
/*加入收藏夹2*/
function _gscms_AddFavorite2(sURL, sTitle) {
    try {
...
/*设为首页*/
function _gscms_SetHome(obj, url) {
    try {
...
function _gscms_GetPageContent(totalCount, pageSize, currentPage)
{
    ...
    for (var i = 1; i <= pageFoot; i++)
    {
        if (i == currentPage)
            ...
            _html += "<a target='_self' href='javascript:;' onclick='_gscms_GetPageContent(" + totalCount + "," + pageSize + "," + i + ")'>" + i + "</a>";
    }
    _html += "</div>";
    $("#PageBar").html(_html);
...
function _gscms_SearchBar(type){
    ...
    _document +="        </div>";
    _document +="    </form>";
    _document +="    <div id=\"search_bar_weather\" class=\"search_bar_right\"></div><script>GSCMS.Core.ShowWeather(\"search_bar_weather\");</script>";
    _document +="</div>";
    return _document;
}
...
var PluginPageVisit	= true;//网站访问统计插件
...
function ajaxPluginVoteAdd(id,mtype,btn)
{
    ...
    $.ajax({
        ...
        url:        _gscms_Host() + site.Dir + "extends/vote/ajax.aspx?oper=ajaxPluginVoteAdd",
        error:        function(XmlHttpRequest,textStatus, errorThrown){if(XmlHttpRequest.responseText!=""){alert(XmlHttpRequest.responseText);}},
        success:    function(d){
            if(d=="ok")
            {
